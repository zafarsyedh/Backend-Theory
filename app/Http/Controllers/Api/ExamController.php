...
class ExamController extends Controller
{
        ...
        try {
            ...
            if ($resData->count() > 0) {
                if ($request->getType == 1) {
                    return Helper::successWithData($resData, 'Practice results');
                } else {
    ...
    public function examSystemStatusUpdate(Request $request)
    {
        try {
            ...
            if ($request->exam_id) {
                $examUpdate = $this->exam->updateExamScheduleStatus($request->exam_id, $request->closeType);
            }
    ...
    public function checkPracticeAttemptComplete(Request $request)
    {
        try {
            ...
            if ($data['commonSolved'] == 1 and $data['specificSolved'] == 1 and $data['videoSolved'] == 1) {
                ...
                $request->merge(['closeType' => 3]);

                $this->savePracticeResult($request);
    ...
    public function savePracticeResult(Request $request)
    {
        ...
        $request->merge(['exam_id' => $request->exam_id]);
        $request->merge(['getType' => 1]);

        $res = $this->getPracticeResult($request);

        $obtainPercentage = $res['data'][0]['percentage'];
        $exam = ExamSchedule::find(1);
        $requirePercentage = $exam->course->courseConfig->practice_persontage;


        $practiceResult = PracticeResult::updateOrCreate(
            [
                'exam_id' => $request->exam_id,
            ],
            [
                'std_id' => $exam->student->id,
                'traffic_id' => $exam->student->traffic_id,
                'exam_id' => $request->exam_id,
                'require_percentage' => $requirePercentage,
                'obtain_percentage' => $obtainPercentage,
            ]
        );

        if ($obtainPercentage >= $requirePercentage) {
            $student = Student::find($exam->student->id);
            $student->is_eligible = 1;
            $student->save();
        }
        return Helper::success([], 'Result created');
    }

    //markEligible
    public function markEligible(Request $request)
    {
        if ($std = Student::where('traffic_id', $request->traffic_id)->latest('id')->first()) {
            $std->is_eligible = 1;
            $std->save();
            return Helper::success([], 'Mark as eligible successfully');
        } else {
            return Helper::error('student not exist', []);
        }
    }
}
